...
if folder_path:
    ...
    if not csv_files:
        ...
        for csv_file in csv_files:
            ...
            with st.spinner(f"Analyzing data from '{csv_file}' with Mistral AI..."):
                try:
                    analysis = query_mistral(prompt)
                    
                    # Display the AI analysis
                    st.subheader(f"AI Analysis: {csv_file}")
                    st.markdown(analysis)
                    
                except Exception as e:
                    st.error(f"Error querying Mistral AI for '{csv_file}': {str(e)}")
